...
#
# A script to download and build czmq for iOS, macOS, and tvOS
#
...
LIBZMQ_DIST=File.join(ROOTDIR, "../libzmq-darwin/dist")
...
# root directory
LIBDIR                  = File.join(SCRIPTDIR, "build/czmq")
...
def download_and_extract_czmq()
  puts "Downloading czmq source"
  pkg_name      = "czmq-#{PKG_VER}"
  pkg           = "#{pkg_name}.tar.gz"
  url           = "https://github.com/zeromq/czmq/releases/download/v#{PKG_VER}/#{pkg}"
  exit 1 unless system("cd #{BUILDDIR} && curl -O -L #{url}")
  exit 1 unless system("cd #{BUILDDIR} && tar xzf #{pkg}")
  FileUtils.mv "#{BUILDDIR}/#{pkg_name}", "build/czmq"
  FileUtils.rm "#{BUILDDIR}/#{pkg}"
end

# Download and extract czmq
download_and_extract_czmq()
...
for platform in PLATFORMS
  ...
  for arch in archs
    ...
    LIBZMQ_INC = "#{LIBZMQ_DIST}/#{platform.downcase}/include"
    LIBZMQ_LIB = "#{LIBZMQ_DIST}/#{platform.downcase}/lib"

    other_cppflags = "-Os -I#{LIBZMQ_INC} -I#{LIBSODIUM_INC} -fembed-bitcode"
    OTHER_CXXFLAGS = "-Os"
    other_ldflags = "-L#{LIBZMQ_LIB} -L#{LIBSODIUM_LIB} -lzmq -lsodium"
    zmq_cflags = "-arch #{arch} #{other_cppflags}"
    zmq_libs = "-arch #{arch} -L#{LIBZMQ_LIB} -L#{LIBSODIUM_LIB}"
    ...
    when "iOS-arm64"
      ...
      ENV["libzmq_CFLAGS"] = zmq_cflags
      ENV["libzmq_LIBS"] = zmq_libs
      ENV["zmq_CFLAGS"] = zmq_cflags
      ENV["zmq_LIBS"] = zmq_libs
    when "tvOS-arm64"
    ...
    configure_cmd = [
      ...
      "--enable-static",
      "--host=#{host}",
      "--with-libzmq=${LIBZMQ_DIST}/#{platform.downcase}",
      "--with-uuid=no",
      "--with-libsystemd=no",
      "--with-liblz4=no",
      "--with-libcurl=no",
